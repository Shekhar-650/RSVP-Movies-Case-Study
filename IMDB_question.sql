...
SELECT
    ...
    COUNT(CASE WHEN year IS NULL THEN 1 END) AS null_count_year,
	COUNT(CASE WHEN date_published IS NULL THEN 1 END) AS null_count_date_published,
	...
	COUNT(CASE WHEN worlwide_gross_income IS NULL THEN 1 END) AS null_count_worlwide_gross_income,
	COUNT(CASE WHEN languages IS NULL THEN 1 END) AS null_count_languages,
	COUNT(CASE WHEN production_company IS NULL THEN 1 END) AS null_count_production_company
FROM movie;

/* ----------------------------------------------
Solutions:-
Below four tables contains null values
...
CREATE VIEW movies_with_one_genre AS
SELECT
    movie_id,
    COUNT(genre) AS Genre_count
FROM genre
...
SELECT COUNT(*) AS count_movies_genre
FROM movies_with_one_genre;
...
-- Q16. If the movies released between 1 April 2018 and 1 April 2019, how many were given a median rating of 8?
-- Type your code below:
...
(
	SELECT SUM(total_votes) AS German_total_votes_count
    FROM ratings AS r
		INNER JOIN movie AS m
			ON r.movie_id = m.id
...
(
	SELECT SUM(total_votes) AS Italian_total_votes_count
    FROM ratings AS r
		INNER JOIN movie AS m
			ON r.movie_id = m.id
...
SELECT *
FROM German_total_votes 
INNER JOIN Italian_total_votes;
